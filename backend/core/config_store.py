...
async def get_user_llm_config(user_id: int) -> dict | None:
    ...
    if has_access_mode_column and has_image_config and has_model_column and has_image_model_column:
        cols = "llm_access_mode, provider, api_key, base_url, image_provider, image_api_key, model, image_model"
        if has_image_base_url_column:
            cols += ", image_base_url"
        cursor = await db.execute(
            f"SELECT {cols} FROM user_llm_config WHERE user_id = ?",
            (user_id,),
        )
...
async def save_user_llm_config(
    ...
    try:
        if has_access_mode_column and has_image_config and has_image_model_column and has_image_base_url_column:
            await db.execute(
        ...
        elif has_access_mode_column and has_image_config and has_image_model_column:
            await db.execute(

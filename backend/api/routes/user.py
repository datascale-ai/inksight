...
async def save_user_llm_config_route(body: dict, user_id: int = Depends(require_user)):
    ...
    if image_base_url and not (image_base_url.startswith("http://") or image_base_url.startswith("https://")):
        return JSONResponse({"error": "image_base_url 必须以 http:// 或 https:// 开头"}, status_code=400)

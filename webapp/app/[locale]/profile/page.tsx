...
export default function ProfilePage() {
  ...
  const handleSaveLlmConfig = async () => {
    ...
    if (llmAccessMode === "preset") {
      if (!llmProvider.trim()) missing.push(tr("文本生成 API 服务商", "Text Generation API Provider"));
    ...
    try {
      ...
      const res = await fetch("/api/user/profile/llm", {
        ...
        body: JSON.stringify({
          ...
          image_provider: llmAccessMode === "custom_openai" ? (imageProvider.trim() || "aliyun") : "aliyun",
          ...
          <CardContent className="space-y-6">
            ...
            {quotaMode === "custom" && (
              <div className="space-y-4">
                ...
                {llmAccessMode === "preset" ? (
                  <>
                    <p className="text-xs font-medium text-ink-light mb-1">{tr("文本生成", "Text Generation")}</p>
                    ...
                    <div className="border-t border-ink/10 mt-3 pt-3">
                      <p className="text-xs font-medium text-ink-light mb-1">{tr("图像生成", "Image Generation")}</p>
                    </div>
                    <Field label={tr("API 服务商", "API Provider")}>
                      <input
                        type="text"
                        value="aliyun"
                        disabled
                        className="w-full rounded-sm border border-ink/20 px-3 py-2 text-sm bg-gray-100 text-ink-light cursor-not-allowed"
                      />
                    </Field>
                    <Field label={tr("模型名称", "Model Name")}>
                      <select
                    ...
                    <Field label={tr("API Key", "API Key")}>
                      <input
                        ...
                        placeholder={tr("（可选）留空会使得调用文生图模型失败", "(Optional) Leaving empty will cause image generation to fail")}
                        className="w-full rounded-sm border border-ink/20 px-3 py-2 text-sm bg-white font-mono"
                        autoComplete="off"
                      />
                    </Field>
                  ...
                  <>
                    ...
                    <Field label={tr("Base URL", "Base URL")}>
                      <input
                        ...
                        placeholder={tr("（可选）例如：https://dashscope.aliyuncs.com/compatible-mode/v1", "(Optional) e.g. https://dashscope.aliyuncs.com/compatible-mode/v1")}
                        className="w-full rounded-sm border border-ink/20 px-3 py-2 text-sm bg-white font-mono"
                      />
                    </Field>
                    <Field label={tr("模型名称", "Model Name")}>
                      <input
                        ...
                        value={imageModel}
                        onChange={(e) => setImageModel(e.target.value)}
                        placeholder={tr("（可选）例如：qwen-image-plus", "(Optional) e.g. qwen-image-plus")}
                        className="w-full rounded-sm border border-ink/20 px-3 py-2 text-sm bg-white font-mono"
                      />
                    </Field>
                    <Field label={tr("API Key", "API Key")}>
